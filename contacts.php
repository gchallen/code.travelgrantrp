...
if ($Me->privChair || ($Me->isPC && $Conf->blindSubmission() == BLIND_NEVER))
    $tOpt["au"] = "Contact authors of submitted applications";
if ($Me->privChair
    || ($Me->isPC && $Conf->timeReviewerViewDecision()))
    $tOpt["auacc"] = "Contact authors of accepted applications";
if ($Me->privChair
    || ($Me->isPC && $Conf->blindSubmission() == BLIND_NEVER && $Conf->timeReviewerViewDecision()))
    $tOpt["aurej"] = "Contact authors of rejected applications";
if ($Me->privChair) {
    $tOpt["auuns"] = "Contact authors of non-submitted applications";
    $tOpt["all"] = "All users";
}
...
if ($Me->privChair && $_REQUEST["t"] == "pc")
    $Conf->infoMsg("<p><a href='account$ConfSiteSuffix?new=1&amp;pc=1' class='button'>Add grant chair</a></p><p>Select a grant chair's name to edit his/her profile or remove him/her from the grant chair list.</p>");
