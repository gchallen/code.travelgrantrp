...
function confHeader() {
    ...
    if ($prow)
	$title = "Application #$prow->paperId";
    else
...
function loadRows() {
	...
	$editRrowLogname = "Granted Amount Review " . $paperTable->editrrow->reviewId;
    else if ($paperTable->editrrow)
	$editRrowLogname = "Granted Amount Review " . $paperTable->editrrow->reviewId . " by " . $paperTable->editrrow->email;
}
...
if (isset($_REQUEST['uploadForm']) && fileUploaded($_FILES['uploadedFile'], $Conf)) {
	...
	$rf->tfError($tf, true, "This review form is for application #" . $req['paperId'] . ", not application #$prow->paperId; did you mean to upload it here?  I have ignored the form.<br /><a class='button_small' href='review$ConfSiteSuffix?p=" . $req['paperId'] . "'>Review application #" . $req['paperId'] . "</a> <a class='button_small' href='offline$ConfSiteSuffix'>General review upload site</a>");
	...
	if ($rf->checkRequestFields($req, $paperTable->editrrow, $tf)) {
		...
		$tf['confirm'][] = "Uploaded review for application #$prow->paperId.";
	}
    }
...
if (!$viewAny && !$editAny) {
	...
	$Conf->errorMsg("You can't see the reviews for this application.  " . whyNotText($whyNotView, "review"));
